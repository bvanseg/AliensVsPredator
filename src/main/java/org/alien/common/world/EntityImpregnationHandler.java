    ...
    public void tick(LivingUpdateEvent event)
    {
        ...
        if (!world.isRemote) {
            ...
            if (ticksUntilBirth < gestationPeriod * stageOnePercentage) {
                int hungerAmplifier = (int) MDXMath.map(age, gestationPeriod * (1 - stageOnePercentage), gestationPeriod, 1, 100);
                host.addPotionEffect(new PotionEffect(MobEffects.HUNGER, ticksUntilBirth, hungerAmplifier));
            }
            ...
            if (ticksUntilBirth < gestationPeriod * stageTwoPercentage) {
                host.addPotionEffect(new PotionEffect(MobEffects.WEAKNESS, ticksUntilBirth));
            }
            ...
            if (ticksUntilBirth < gestationPeriod * stageThreePercentage) {
                host.addPotionEffect(new PotionEffect(MobEffects.SLOWNESS, ticksUntilBirth, stageThreeAmplifier));
                host.addPotionEffect(new PotionEffect(MobEffects.MINING_FATIGUE, ticksUntilBirth, stageThreeAmplifier));
            }
            ...
            if (ticksUntilBirth < gestationPeriod * stageFourPercentage) {
                host.addPotionEffect(new PotionEffect(MobEffects.NAUSEA, ticksUntilBirth, stageFourAmplifier));
                host.addPotionEffect(new PotionEffect(MobEffects.BLINDNESS, ticksUntilBirth, stageFourAmplifier));
            }
